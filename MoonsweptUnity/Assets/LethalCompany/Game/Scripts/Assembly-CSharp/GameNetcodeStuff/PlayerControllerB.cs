...
{if(!base.IsOwner)				{
		...
		{
			if ((jetpackControls || disablingJetpackControls || isClimbingLadder) && updatePlayerLookInterval > 0.15f)
			{
				updatePlayerLookInterval = 0f;
		...
		private void Update()
		{
			if ((base.IsOwner && isPlayerControlled && (!base.IsServer || isHostPlayerObject)) || isTestingPlayer)
			{
				...
				if (!inSpecialInteractAnimation || inShockingMinigame || StartOfRound.Instance.suckingPlayersOutOfShip)
				{
					...
					float num8 = Vector3.Angle(playerGroundNormal, Vector3.up);
					if (playerSlideForce.magnitude < 0.15f)
					{
						if (thisController.isGrounded && num8 > thisController.slopeLimit && fallValue <= -12f && fallValue > -43f)
						{
							float num9 = (num8 - thisController.slopeLimit) / 25f * 2f;
							playerSlideForce = playerGroundNormal * bumpWallForce * (Mathf.Abs(fallValue + 5f) * 0.4f) * num9;
							playerSlideForce.y = 0f;
							playerSlideForce.x = Mathf.Min(Mathf.Abs(fallValue * 0.6f), playerSlideForce.x);
							playerSlideForce.z = Mathf.Min(Mathf.Abs(fallValue * 0.6f), playerSlideForce.z);
							if (playerSlideForce.magnitude < 2f)
							{
								playerSlideForce = Vector3.zero;
							}
							else
							{
								timeAtLastWallBump = Time.realtimeSinceStartup;
							}
						}
						else
						{
					...
					{
						playerSlideForce = Vector3.Lerp(playerSlideForce, Vector3.zero, 5f * Time.deltaTime);
					}
					if (!inSpecialInteractAnimation || inShockingMinigame)
					{
						if (!thisController.isGrounded || (Time.realtimeSinceStartup - timeAtLastWallBump < 0.35f && num8 > thisController.slopeLimit))
						{
							if (jetpackControls && !disablingJetpackControls)
							{
							...
							else if (Time.realtimeSinceStartup - timeAtLastWallBump > 0.175f)
							{
							...
							if (!isJumping && !isFallingFromJump)
							{
								if (!isFallingNoJump)
								{
									...
									if (Time.realtimeSinceStartup - timeAtLastWallBump > 0.25f && playerSlideForce.magnitude < 3f)
									{
										fallValue = -7f;
			...
			{
				if ((!isClimbingLadder && !inShockingMinigame) || freeRotationInInteractAnimation)
				{
